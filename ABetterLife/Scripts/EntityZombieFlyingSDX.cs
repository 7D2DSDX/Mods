...
internal class EntityZombieFlyingSDX : EntityVulture
{
    ...
    private readonly EAITaskList TargetList = new EAITaskList();

    // Create local task and target lists, so we can add more fleshed out flying creatures
    private readonly EAITaskList TaskList = new EAITaskList();
    ...
    public override void PostInit()
    {
        ...
        if (IsAlive() && (int)lifetime > 0 && (int)lifetime < 9999999 && !world.IsRemote())
        {
            ...
            if (player != null)
            {
                ...
                hunterLevel = (player as EntityPlayer).inventory.holdingItemData.itemValue.UseTimes;
            ...
            while (enumerator.MoveNext())
            {
                ...
                if ((_other as EntityPlayer).inventory.holdingItem.Name == tameItem)
                    if ((_other as EntityPlayer).inventory.holdingItemItemValue.Meta == uid)
                    {
                        ...
                        hunterLevel = (_other as EntityPlayer).inventory.holdingItemItemValue.UseTimes;
    ...
    // just because I want to be able to modify speed...
    public override void MoveEntityHeaded(Vector3 _direction)
    {
        if (AttachedToEntity != null) return;
        if (IsDead())
        {
            entityCollision(motion);
            motion.y = motion.y - 0.08f;
            motion.y = motion.y * 0.98f;
            motion.x = motion.x * 0.91f;
            motion.z = motion.z * 0.91f;
            return;
        }

        if (IsInWater())
        {
            Move(_direction, 0.02f, 1f);
            entityCollision(motion);
            motion *= 0.8f;
        }
        else if (IsInLava())
        ...
        {
            // didn't want to waste too much time here, so I just added a modifier for "base" speed...
            var num = 0.91f;
            if (onGround)
            {
                num = 0.55f;
                var type = world.GetBlock(Utils.Fastfloor(position.x), Utils.Fastfloor(boundingBox.min.y) - 1,
                    Utils.Fastfloor(position.z)).type;
                if (type > 0) num = Mathf.Clamp(Block.list[type].blockMaterial.Friction, 0.01f, 1f);
            }

            var num2 = 0.163f / (num * num * num);
            var baseSpeed = !onGround ? 0.02f : 0.1f * num2;
            var baseMaxSpeed = 1f;
            if (base.GetRevengeTarget() != null && isHunting)
            {
                baseSpeed = !onGround ? 0.005f : 0.025f * num2;
                baseMaxSpeed = 0.5f;
            }

            Move(_direction, baseSpeed, baseMaxSpeed);
            entityCollision(motion);
            motion *= num;
        }
    }
    ...
    public override void CopyPropertiesFromEntityClass()
    {
        ...
        while (entityClass.Properties.Values.ContainsKey(EntityClass.PropAITask + num))
        {
            ...
            if (entityClass.Properties.Params1.ContainsKey(EntityClass.PropAITask + num))
                eaibase.SetPar1(entityClass.Properties.Params1[EntityClass.PropAITask + num]);
            if (entityClass.Properties.Params2.ContainsKey(EntityClass.PropAITask + num))
                eaibase.SetPar2(entityClass.Properties.Params2[EntityClass.PropAITask + num]);
        ...
        while (entityClass.Properties.Values.ContainsKey(EntityClass.PropAITargetTask + num2))
        {
            ...
            if (entityClass.Properties.Params1.ContainsKey(EntityClass.PropAITargetTask + num2))
                eaibase2.SetPar1(entityClass.Properties.Params1[EntityClass.PropAITargetTask + num2]);
            if (entityClass.Properties.Params2.ContainsKey(EntityClass.PropAITargetTask + num2))
                eaibase2.SetPar2(entityClass.Properties.Params2[EntityClass.PropAITargetTask + num2]);
        ...
        if (AttackTimeout <= 0)
        {
            ...
            if (sqrMagnitude < 1f || sqrMagnitude > 6400f)
            {
                if (!isWithinHomeDistanceCurrentPosition() && GetMasterEntity() == null && !isHunting)
                {
                    ...
                    var ye = RandomPositionGenerator.CalcTowards(this, 2 * getMaximumHomeDistance(),
                        2 * getMaximumHomeDistance(), getHomePosition().position.ToVector3());
